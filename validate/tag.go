...
// comma is not a rule.
//
// The error says what is wrong with the tag rather than what is wrong with a
// request. It reaches the caller as an internal error naming the field, because
// a tag is written by a programmer and the sentence is for them.
...
// tagSafe is whether a name can be written in a tag and read back out of it.
//
// A rule whose name carries a separator would have to be escaped at every use,
// which is a rule nobody would write down, so [Register] refuses one rather
// than letting it be registered and never matched.
