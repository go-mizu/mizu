...
// openCluster creates a cluster store that routes to remote zebra nodes.
func openCluster(_ context.Context, u *url.URL) (*clusterStore, error) {
	q := u.Query()
	peers := strings.Split(q.Get("peers"), ",")
	if len(peers) == 0 {
		return nil, fmt.Errorf("zebra: no peers specified")
	}
	...
	for _, addr := range peers {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("zebra: connect to %s: %w", addr, err)
		}
...
// clusterStore routes operations to remote zebra nodes via rendezvous hashing.
...
func (b *clusterBucket) List(_ context.Context, prefix string, limit, offset int, opts storage.Options) (storage.ObjectIter, error) {
...
// remoteNode is a TCP client to a single zebra node.
...
func (rn *remoteNode) put(bucket, key, contentType string, src io.Reader, size int64) (*storage.Object, error) {
	...
	if status != statusOK {
		return nil, fmt.Errorf("zebra: remote put failed (status %d)", status)
	}
...
func (rn *remoteNode) get(bucket, key string, offset, length int64) (io.ReadCloser, *storage.Object, error) {
	...
	if status != statusOK {
		return nil, nil, fmt.Errorf("zebra: remote get failed (status %d)", status)
	}

	// Parse response: [8B size][2B ct_len][ct][8B created][8B updated][value]
	if len(respBody) < 26 {
		return nil, nil, fmt.Errorf("zebra: truncated get response")
	}
	...
	return acquireReader(value), obj, nil
}
...
func (rn *remoteNode) stat(bucket, key string) (*storage.Object, error) {
	...
	if status != statusOK {
		return nil, fmt.Errorf("zebra: remote stat failed (status %d)", status)
	}

	// Parse: [8B size][2B ct_len][ct][8B created][8B updated]
	if len(respBody) < 26 {
		return nil, fmt.Errorf("zebra: truncated stat response")
	}
	objSize := int64(binary.BigEndian.Uint64(respBody[0:8]))
	ctLen := int(binary.BigEndian.Uint16(respBody[8:10]))
	ct := string(respBody[10 : 10+ctLen])
...
func (rn *remoteNode) del(bucket, key string) error {
	...
	if status != statusOK {
		return fmt.Errorf("zebra: remote delete failed (status %d)", status)
	}
	return nil
}
...
func (rn *remoteNode) list(bucket, prefix string, recursive bool) ([]*storage.Object, error) {
	...
	if status != statusOK {
		return nil, fmt.Errorf("zebra: remote list failed (status %d)", status)
	}
...
// NodeServer serves the zebra binary protocol for a standalone node.
