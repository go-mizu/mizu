...
// Tiered sync.Pools for pwrite buffers.
...
func getWriteBuf(size int64) ([]byte, *[]byte, int) {
	tiers := [5]int64{64*1024 + 1024, 1024*1024 + 1024, 10*1024*1024 + 1024, 100*1024*1024 + 4096, 256 * 1024 * 1024}
...
func newVolume(path string, prealloc int64) (*volume, error) {
	...
	if err := os.MkdirAll(dir, 0o750); err != nil {
		return nil, fmt.Errorf("zebra: mkdir %q: %w", dir, err)
	}

	fd, err := os.OpenFile(path, os.O_CREATE|os.O_RDWR, 0o600)
	if err != nil {
		return nil, fmt.Errorf("zebra: open volume: %w", err)
	}

	info, err := fd.Stat()
	if err != nil {
		fd.Close()
		return nil, fmt.Errorf("zebra: stat volume: %w", err)
	}
	...
	if info.Size() < allocSize {
		if err := fd.Truncate(allocSize); err != nil {
			fd.Close()
			return nil, fmt.Errorf("zebra: truncate volume: %w", err)
		}
	}
	...
	if err != nil {
		fd.Close()
		return nil, fmt.Errorf("zebra: mmap: %w", err)
	}
...
func (v *volume) readHeader() error {
	...
	if len(r.buf) < headerSize {
		return errors.New("zebra: volume too small for header")
	}
	if string(r.buf[0:8]) != magic {
		return errors.New("zebra: invalid volume magic")
	}
	ver := binary.LittleEndian.Uint32(r.buf[8:12])
	if ver != version {
		return fmt.Errorf("zebra: unsupported version %d", ver)
	}
...
func (v *volume) appendRecord(recType byte, bucket, key, contentType string, value []byte, timestamp int64) (int64, int64, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("zebra: pwrite: %w", err)
	}
	return offset, offset + int64(valPos), nil
}
...
func (v *volume) writeFromReader(recType byte, bucket, key, contentType string, src io.Reader, size int64, timestamp int64) (int64, error) {
		...
		if size > 0 {
			if _, err := io.ReadFull(src, r.buf[valOff:valOff+size]); err != nil {
				if err != io.EOF && err != io.ErrUnexpectedEOF {
					return 0, fmt.Errorf("zebra: read value: %w", err)
				}
			}
		}
		pos += int(size)
	...
	putWriteBuf(bp, poolIdx)
	if err != nil {
		return 0, fmt.Errorf("zebra: pwrite: %w", err)
	}
	return valOff, nil
}
...
func (v *volume) recover(idx *index, inlineMax int64) error {
	r := v.region.Load()
	tail := v.tail.Load()
	...
	for pos < tail {
		...
		case recPut:
			...
			// Rebuild inline cache for small values.
			if inlineMax > 0 && vl <= inlineMax {
...
func (v *volume) growFile(needed int64) error {
	...
	if err := v.fd.Truncate(newSize); err != nil {
		return fmt.Errorf("zebra: truncate: %w", err)
	}
...
func (v *volume) sync() error {
	...
	if errno != 0 {
		return fmt.Errorf("zebra: msync: %w", errno)
	}
	return nil
}
