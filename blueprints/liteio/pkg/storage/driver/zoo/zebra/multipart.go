...
func (b *bucket) InitMultipart(_ context.Context, key string, contentType string, _ storage.Options) (*storage.MultipartUpload, error) {
...
func (b *bucket) UploadPart(_ context.Context, mu *storage.MultipartUpload, number int, src io.Reader, size int64, _ storage.Options) (*storage.PartInfo, error) {
	if number <= 0 || number > 10000 {
		return nil, fmt.Errorf("zebra: part number %d out of range (1-10000)", number)
	}
...
func (b *bucket) CompleteMultipart(_ context.Context, mu *storage.MultipartUpload, parts []*storage.PartInfo, _ storage.Options) (*storage.Object, error) {
	if len(parts) == 0 {
		return nil, fmt.Errorf("zebra: no parts to complete")
	}
	...
	for _, part := range sortedParts {
		...
		if !exists {
			b.st.mp.mu.Unlock()
			return nil, fmt.Errorf("zebra: part %d not found", part.Number)
		}
	...
	// Write concatenated data — use appropriate stripe.
	now := fastNow()
	...
	e.created = now
	e.updated = now
	st.idx.put(b.name, upload.mu.Key, e)
