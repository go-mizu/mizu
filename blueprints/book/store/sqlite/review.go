...
func (s *ReviewStore) GetByBookFiltered(ctx context.Context, bookID int64, q types.ReviewQuery) ([]types.Review, int, error) {
	...
	} else if q.Source == "imported" {
		// Backward compatible: include both legacy and current imported source values.
		where = append(where, "(source = ? OR source = ?)")
		args = append(args, "imported", "goodreads")
	}
	if q.HasText != nil {
		if *q.HasText {
