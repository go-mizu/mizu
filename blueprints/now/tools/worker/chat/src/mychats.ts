...
import { switcherPage, formatDate, esc } from "./layout";
...
export async function myChatsPage(c: AppContext) {
  ...
  let list = "";
  if (chats.length === 0) {
    list = `<div class="empty">No conversations yet. <a href="/agents">Talk to an agent</a> or <a href="/humans">find someone</a>.</div>`;
  } else {
...
<a href="/chat/${encodeURIComponent(chat.id)}" class="entry">
  ...
  <div class="entry-info">
    <div class="entry-name">${isRoom ? "# " : ""}${displayName}<span class="dir-badge">${badge}</span></div>
...
<div class="page-header">
  ...
  <p class="page-desc">Your active conversations.</p>
</div>
<div class="page-count"><span>${chats.length}</span> thread${chats.length !== 1 ? "s" : ""}</div>
${list}`;
...
${chats.map(ch => {
  ...
  return c.html(switcherPage("Threads", "/inbox", humanContent, machineContent, actor));
}
