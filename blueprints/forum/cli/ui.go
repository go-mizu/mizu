...
// Color palette (forum theme)
var (
	primaryColor   = lipgloss.Color("#FF4500") // Reddit orange
...
func (u *UI) UserRow(username, email string, admin, suspended bool) {
	badges := ""
	if admin {
		badges += badgeStyle.Render(" [admin]")
	}
	if suspended {
		badges = errorStyle.Render(" [suspended]")
	}

	fmt.Printf("  %s %-20s %s%s\n",
		iconUser,
		usernameStyle.Render("@"+username),
		valueStyle.Render(email),
		badges)
}

// BoardRow prints a formatted board row
func (u *UI) BoardRow(name, title string, memberCount int) {
	members := subtitleStyle.Render(fmt.Sprintf("(%d members)", memberCount))
	fmt.Printf("  %s %-20s %s %s\n",
		iconBoard,
		boardStyle.Render("b/"+name),
		valueStyle.Render(title),
		members)
}

// ThreadRow prints a formatted thread row
func (u *UI) ThreadRow(title, author string, score, commentCount int) {
	meta := subtitleStyle.Render(fmt.Sprintf("[%+d] %d comments", score, commentCount))
	if len(title) > 40 {
		title = title[:40] + "..."
	}
	fmt.Printf("  %s %-45s by %s %s\n",
		iconThread,
		valueStyle.Render(title),
		usernameStyle.Render("@"+author),
		meta)
}

// TableHeader prints a table header
func (u *UI) TableHeader(columns ...string) {
	fmt.Print("  ")
	for _, col := range columns {
		fmt.Printf("%-20s ", labelStyle.Render(col))
	}
	fmt.Println()
	fmt.Print("  ")
	fmt.Println(subtitleStyle.Render(strings.Repeat("─", 60)))
}
