...
func NewClient() *Client {
	return &Client{
		...
		userAgent:   defaultUA,
		rateLimit:   100 * time.Millisecond, // Be polite even without rate limits
...
func (c *Client) FetchFeed(ctx context.Context, feed FeedType) ([]int, error) {
	url := fmt.Sprintf("%s/%s.json", baseURL, feed.FeedEndpoint())
...
// FetchItemsSince fetches items from startID up to limit items.
// Returns fetched items and the next ID to resume from.
func (c *Client) FetchItemsSince(ctx context.Context, startID int, limit int) ([]*Item, int, error) {
	maxID, err := c.FetchMaxItem(ctx)
	if err != nil {
	...
	return items, endID + 1, err
}

// FetchSubreddit implements seed.Source - returns HN as a single "board".
func (c *Client) FetchSubreddit(ctx context.Context, name string) (*seed.SubredditData, error) {
	return &seed.SubredditData{
		...
		Subscribers: 0, // Not applicable for HN
	}, nil
}

// FetchThreads implements seed.Source - fetches stories from HN.
func (c *Client) FetchThreads(ctx context.Context, _ string, opts seed.FetchOpts) ([]*seed.ThreadData, error) {
	...
	threads := make([]*seed.ThreadData, 0, len(items))
	for _, item := range items {
		if item == nil || item.IsDeleted() || !item.IsStory() {
			continue
		}

		thread := c.itemToThread(item)
...
func (c *Client) FetchComments(ctx context.Context, _, threadID string) ([]*seed.CommentData, error) {
	...
	return c.fetchCommentTree(ctx, story.Kids, threadID, "", 0)
}

// fetchCommentTree recursively fetches comments and their replies.
func (c *Client) fetchCommentTree(ctx context.Context, ids []int, threadID, parentID string, depth int) ([]*seed.CommentData, error) {
	...
	for _, item := range items {
		...
		comment := c.itemToComment(item, threadID, parentID, depth)

		// Recursively fetch replies
		if len(item.Kids) > 0 {
			replies, err := c.fetchCommentTree(ctx, item.Kids, threadID, strconv.Itoa(item.ID), depth+1)
			if err == nil {
...
// itemToThread converts an HN Item to seed.ThreadData.
func (c *Client) itemToThread(item *Item) *seed.ThreadData {
	// Determine if it's a text post (Ask HN, etc.) or a link
	isSelf := item.URL == ""
	content := c.htmlToText(item.Text)

	return &seed.ThreadData{
		...
		UpvoteCount:  int64(item.Score), // HN only shows score, not up/down separately
...
func (c *Client) itemToComment(item *Item, threadID, parentID string, depth int) *seed.CommentData {
