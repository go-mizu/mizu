...
func NewInit() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Initialize the Email SQLite database with all required tables.
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	fmt.Println("  1. Run 'email seed' to add sample data")
	fmt.Println("  2. Run 'email serve' to start the server")
	fmt.Println()
