...
// AttachmentsStore provides PostgreSQL-based attachment storage.
...
func (s *AttachmentsStore) Create(ctx context.Context, att *attachments.Attachment) error {
	...
	`, att.ID, att.RecordID, att.FieldID, att.Filename, att.Size, att.MimeType, att.URL, nullString(att.ThumbnailURL), nullInt(att.Width), nullInt(att.Height), att.CreatedAt)
	return err
}
