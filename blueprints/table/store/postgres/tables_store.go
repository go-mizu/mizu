...
// TablesStore provides PostgreSQL-based table storage.
...
func (s *TablesStore) Create(ctx context.Context, tbl *tables.Table) error {
	...
	`, tbl.ID, tbl.BaseID, tbl.Name, nullString(tbl.Description), nullString(tbl.Icon), tbl.Position, nullString(tbl.PrimaryFieldID), tbl.AutoNumberSeq, tbl.CreatedBy, tbl.CreatedAt, tbl.UpdatedAt)
	return err
}
...
func (s *TablesStore) Update(ctx context.Context, tbl *tables.Table) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		WHERE id = $8
	`, tbl.Name, nullString(tbl.Description), nullString(tbl.Icon), tbl.Position, nullString(tbl.PrimaryFieldID), tbl.AutoNumberSeq, tbl.UpdatedAt, tbl.ID)
	return err
}
