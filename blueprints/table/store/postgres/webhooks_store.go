...
// WebhooksStore provides PostgreSQL-based webhook storage.
...
func (s *WebhooksStore) Create(ctx context.Context, webhook *webhooks.Webhook) error {
	...
	`, webhook.ID, webhook.BaseID, nullString(webhook.TableID), webhook.URL, eventsJSON, nullString(webhook.Secret), webhook.IsActive, webhook.CreatedBy, webhook.CreatedAt)
	return err
}
...
func (s *WebhooksStore) Update(ctx context.Context, webhook *webhooks.Webhook) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		WHERE id = $5
	`, webhook.URL, eventsJSON, nullString(webhook.Secret), webhook.IsActive, webhook.ID)
	return err
}
...
func (s *WebhooksStore) CreateDelivery(ctx context.Context, delivery *webhooks.Delivery) error {
	...
	`, delivery.ID, delivery.WebhookID, delivery.Event, delivery.Payload, nullInt(delivery.StatusCode), nullString(delivery.Response), nullInt(delivery.DurationMs), delivery.CreatedAt)
	return err
}
...
func (s *WebhooksStore) scanWebhook(row *sql.Row) (*webhooks.Webhook, error) {
	...
	err := row.Scan(&webhook.ID, &webhook.BaseID, &tableID, &webhook.URL, &eventsJSON, &secret, &webhook.IsActive, &webhook.CreatedBy, &webhook.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, webhooks.ErrNotFound
	}
	if err != nil {
...
func (s *WebhooksStore) scanWebhooks(rows *sql.Rows) ([]*webhooks.Webhook, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&webhook.ID, &webhook.BaseID, &tableID, &webhook.URL, &eventsJSON, &secret, &webhook.IsActive, &webhook.CreatedBy, &webhook.CreatedAt)
		if err != nil {
			return nil, err
		}
