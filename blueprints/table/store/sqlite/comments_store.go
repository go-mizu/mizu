...
// CommentsStore provides SQLite-based comment storage.
...
func (s *CommentsStore) Create(ctx context.Context, comment *comments.Comment) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, comment.ID, comment.RecordID, comment.ParentID, comment.UserID, comment.Content, comment.IsResolved, comment.CreatedAt, comment.UpdatedAt)
	return err
}
...
func (s *CommentsStore) Update(ctx context.Context, comment *comments.Comment) error {
	...
	_, err := s.db.ExecContext(ctx, `
		UPDATE comments SET
			content = ?, is_resolved = ?, updated_at = ?
		WHERE id = ?
...
func (s *CommentsStore) DeleteByRecord(ctx context.Context, recordID string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM comments WHERE record_id = ?`, recordID)
	return err
}
