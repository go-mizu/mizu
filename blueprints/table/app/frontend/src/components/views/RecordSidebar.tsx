...
export function RecordSidebar({
  ...
  const { fields, updateCellValue, deleteRecord, comments, fetchComments, createComment, currentView } = useBaseStore();
  ...
  const handleExpandFullPage = () => {
    if (onExpandFullPage) {
      ...
      // Default behavior: navigate to record page
      const viewParam = currentView ? `?view=${currentView.id}` : '';
      window.location.href = `/record/${record.id}${viewParam}`;
    }
  };
  ...
  const [sidebarWidth, setSidebarWidth] = useState(() => {
    const savedWidth = localStorage.getItem(`recordSidebarWidth_${currentView?.id || 'default'}`);
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      const newWidth = Math.min(Math.max(startWidth + diff, MIN_WIDTH), maxWidth);
    ...
    const handleMouseUp = () => {
      ...
      localStorage.setItem(`recordSidebarWidth_${currentView?.id || 'default'}`, String(sidebarWidth));
    };

    document.addEventListener('mousemove', handleMouseMove);
    document.addEventListener('mouseup', handleMouseUp);
  }, [sidebarWidth, currentView?.id]);
  ...
  const titleValue = primaryField ? (record.values[primaryField.id] as string) || 'Untitled' : 'Untitled';
  ...
  return (
    <>
      ...
      <div
        ref={sidebarRef}
        className={`fixed right-0 top-0 h-full bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}
      ...
      >
        ...
        <div
          ref={resizeHandleRef}
          className={`absolute left-0 top-0 bottom-0 w-1 cursor-ew-resize hover:bg-primary/50 transition-colors ${isResizing ? 'bg-primary' : ''}`}
