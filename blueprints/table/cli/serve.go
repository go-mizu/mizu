...
func NewServe() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Start the Table web server.

The server provides:
  - Web UI for database management and editing
  - REST API for programmatic access
  - Real-time updates for collaboration

Examples:
  table serve                    # Start on default port 8080
  table serve --addr :3000       # Start on port 3000
  table serve --dev              # Enable development mode`,
		RunE: runServe,
	}
