...
import (
	...
	"github.com/go-mizu/mizu/blueprints/search/pkg/trafilatura/internal/etree"
	"github.com/go-mizu/mizu/blueprints/search/pkg/trafilatura/internal/selector"
...
// extractMetadataFast extracts only the page title, skipping all other metadata.
func extractMetadataFast(doc *html.Node) Metadata {
	var m Metadata
	for _, node := range dom.GetElementsByTagName(doc, "meta") {
		prop := dom.GetAttribute(node, "property")
		if prop == "og:title" {
			if content := dom.GetAttribute(node, "content"); content != "" {
				m.Title = strings.TrimSpace(content)
				return m
			}
		}
		name := dom.GetAttribute(node, "name")
		if name == "title" || name == "twitter:title" {
			if content := dom.GetAttribute(node, "content"); content != "" {
				m.Title = strings.TrimSpace(content)
				return m
			}
	...
	for _, node := range dom.GetElementsByTagName(doc, "title") {
		if text := dom.TextContent(node); text != "" {
			m.Title = strings.TrimSpace(text)
			return m
		}
	}
	for _, node := range dom.GetElementsByTagName(doc, "h1") {
		if text := dom.TextContent(node); text != "" {
			m.Title = strings.TrimSpace(text)
			return m
		}
