...
import (
	...
	"github.com/go-mizu/mizu/blueprints/search/pkg/trafilatura/internal/lru"
...
func ExtractDocument(doc *html.Node, opts Options) (*ExtractResult, error) {
	...
	if opts.SkipFullMetadata {
		metadata = extractMetadataFast(doc)
	} else {
	...
	var docBackup1, docBackup2 *html.Node
	if opts.EnableFallback {
		docBackup1 = dom.Clone(doc, true)
	}
	if !opts.SkipBaselineRescue {
	...
	if !opts.SkipBaselineRescue && lenText < opts.Config.MinExtractedSize && opts.Focus != FavorPrecision {
