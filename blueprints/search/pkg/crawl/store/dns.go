...
classify:
	...
	if lastErr != nil && isTimeoutErr(lastErr) {
		s := &d.shards[shardFor(domain)]
		s.mu.Lock()
		s.timeout[domain] = truncateDNSErr(lastErr)
		s.mu.Unlock()
		d.timedOut.Add(1)
	} else {
...
// truncateDNSErr returns a short error message for storage.
func truncateDNSErr(err error) string {
	if err == nil {
		return "unknown"
	}
	s := err.Error()
...
func (d *DNSResolver) ResolveOne(ctx context.Context, domain string) (ips []string, dead bool, err error) {
	...
	for _, resolver := range d.resolvers {
		...
		if isDefinitelyDead(lookupErr) {
			...
			s.mu.Lock()
			s.dead[domain] = truncateDNSErr(lookupErr)
			s.mu.Unlock()
			d.failed.Add(1)
...
func (d *DNSResolver) Stats() string {
	...
	if cached > 0 {
		return fmt.Sprintf("%d live (%4.1f%%), %d dead, %d timeout, %d cached, took %s",
			ok, pct, fail, tout, cached, d.duration.Truncate(time.Millisecond))
	}
	return fmt.Sprintf("%d live (%4.1f%%), %d dead, %d timeout, took %s",
		ok, pct, fail, tout, d.duration.Truncate(time.Millisecond))
}
...
func (s *dnsCache) Lookup(host string) (string, bool) {
...
func (s *dnsCache) IsDead(host string) bool {
