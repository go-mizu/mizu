// Package postgres_pgsearch provides a PostgreSQL driver using ParadeDB's pg_search extension.
// pg_search implements BM25 full-text search using Tantivy (Rust-based Lucene alternative).
package postgres_pgsearch
...
func init() {
	fineweb.Register("postgres_pgsearch", func(cfg fineweb.DriverConfig) (fineweb.Driver, error) {
		return New(cfg)
	})
}

// DefaultDSN is the default PostgreSQL connection string for ParadeDB.
const DefaultDSN = "postgres://fineweb:fineweb@localhost:5433/fineweb?sslmode=disable"

// DefaultTableName is the default table name.
const DefaultTableName = "documents"

// Driver implements the fineweb.Driver interface using ParadeDB pg_search.
type Driver struct {
	pool      *pgxpool.Pool
	tableName string
	dsn       string
...
// New creates a new ParadeDB pg_search driver.
...
// ensureExtension creates the pg_search extension if it doesn't exist.
func (d *Driver) ensureExtension(ctx context.Context) error {
	_, err := d.pool.Exec(ctx, `CREATE EXTENSION IF NOT EXISTS pg_search`)
	if err != nil {
		return fmt.Errorf("creating pg_search extension: %w", err)
	}
	return nil
}

// ensureTable creates the documents table with BM25 index if it doesn't exist.
...
func (d *Driver) ensureBM25Index(ctx context.Context) error {
	...
	// Create BM25 index using ParadeDB syntax
	// Note: pg_search uses a specific syntax for creating BM25 indexes
...
func (d *Driver) Name() string {
	return "postgres_pgsearch"
}

// Info returns driver metadata.
func (d *Driver) Info() *fineweb.DriverInfo {
	return &fineweb.DriverInfo{
		Name:        "postgres_pgsearch",
		Description: "ParadeDB pg_search with BM25 scoring via Tantivy",
		Features:    []string{"bm25", "full-text-search", "tantivy", "phrase-search", "boolean-search"},
		External:    true,
	}
}
...
func (d *Driver) Search(ctx context.Context, query string, limit, offset int) (*fineweb.SearchResult, error) {
	...
	// Search query using ParadeDB @@@ operator
	// paradedb.score() returns the BM25 score
	searchSQL := fmt.Sprintf(`
		SELECT id, url, content, dump, date, language, language_score,
		       paradedb.score(id) as score
		FROM %s
		WHERE %s @@@ paradedb.parse('%s')
		ORDER BY score DESC
		LIMIT $1 OFFSET $2
	`, d.tableName, d.tableName, escapedQuery)
	...
	countSQL := fmt.Sprintf(`
		SELECT COUNT(*) FROM %s WHERE %s @@@ paradedb.parse('%s')
	`, d.tableName, d.tableName, escapedQuery)
...
func WaitForService(ctx context.Context, dsn string, timeout time.Duration) error {
	...
	return fmt.Errorf("postgres (paradedb) not ready after %v", timeout)
}

// IsServiceAvailable checks if ParadeDB is reachable.
...
func NewWithEnv(cfg fineweb.DriverConfig) (*Driver, error) {
	...
	if dsn := os.Getenv("PARADEDB_DSN"); dsn != "" {
		cfg.Options["dsn"] = dsn
	}
	return New(cfg)
}
