...
func (s *State) Enqueue(rawURL, entityType string, priority int) error {
	_, err := s.db.Exec(`INSERT OR IGNORE INTO queue (url, entity_type, priority) VALUES (?, ?, ?)`, rawURL, entityType, priority)
	return err
}
...
func (s *State) EnqueueBatch(items []QueueItem) error {
	...
	for _, it := range items {
		if _, err := tx.Exec(`INSERT OR IGNORE INTO queue (url, entity_type, priority) VALUES (?, ?, ?)`, it.URL, it.EntityType, it.Priority); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *State) Done(rawURL, entityType string, statusCode int) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
...
func (s *State) ListJobs(limit int) ([]JobRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&job.JobID, &job.Name, &job.Type, &job.Status, &job.StartedAt, &completedAt); err != nil {
			return jobs, err
		}
