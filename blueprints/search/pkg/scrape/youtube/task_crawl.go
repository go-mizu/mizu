...
func (t *CrawlTask) runEntityTask(ctx context.Context, item QueueItem) {
	...
	case EntityVideo:
		_, _ = (&VideoTask{URL: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).Run(ctx, func(*VideoState) {})
	case EntityChannel:
		_, _ = (&ChannelTask{URL: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).Run(ctx, func(*ChannelState) {})
	case EntityPlaylist:
		_, _ = (&PlaylistTask{URL: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).Run(ctx, func(*PlaylistState) {})
	case EntitySearch:
		_, _ = (&SearchTask{Query: item.URL, Enqueue: true, Client: t.Client, DB: t.DB, StateDB: t.StateDB, MaxResults: t.Config.MaxResults}).Run(ctx, func(*SearchState) {})
	default:
