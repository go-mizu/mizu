...
func (t *CrawlTask) runEntityTask(ctx context.Context, item QueueItem) {
	...
	case EntityGuild:
		(&GuildTask{ID: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).
			Run(ctx, func(*GuildState) {})
	case EntityChannel:
		(&ChannelTask{ID: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).
			Run(ctx, func(*ChannelState) {})
	case EntityMessagePage:
		(&MessagesTask{URL: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).
			Run(ctx, func(*MessagesState) {})
	case EntityUser:
		(&UserTask{ID: item.URL, Client: t.Client, DB: t.DB, StateDB: t.StateDB}).
			Run(ctx, func(*UserState) {})
	default:
		if t.StateDB != nil {
