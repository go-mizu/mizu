...
function base64Decode(s: string): Uint8Array {
  ...
  for (let i = 0; i < padded.length; i += 4) {
    const a = lookup[padded[i]] || 0
    const b = lookup[padded[i + 1]] || 0
    const c = lookup[padded[i + 2]] || 0
    const d = lookup[padded[i + 3]] || 0

    const triplet = (a << 18) | (b << 12) | (c << 6) | d

    if (j < outLen) bytes[j++] = (triplet >> 16) & 0xff
    if (j < outLen) bytes[j++] = (triplet >> 8) & 0xff
    if (j < outLen) bytes[j++] = triplet & 0xff
  }
...
function base64Encode(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < len; i += 3) {
    ...
    result += i + 1 < len ? b64chars[(triplet >> 6) & 0x3f] : '='
    result += i + 2 < len ? b64chars[triplet & 0x3f] : '='
  }
