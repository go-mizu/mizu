"""mail.tm API client: create mailbox and poll for OTP."""
...
class MailTmClient:
    ...
    def create_mailbox(self, local: str) -> Mailbox:
        """Create a new mailbox and return credentials + bearer token."""
        domain = self._get_domain()
        address = f"{local}@{domain}"
        password = f"Mz{local[:6]}!9xQ"  # deterministic but valid

        self._log(f"creating mailbox {address}")
        resp = self._client.post(f"{BASE}/accounts", json={"address": address, "password": password})
    ...
    def poll_for_otp(self, mailbox: Mailbox, timeout: int = POLL_TIMEOUT) -> str:
        """Poll inbox until a 6-digit OTP from X/Twitter arrives. Returns the code."""
        ...
        while time.time() < deadline:
            try:
                ...
                for msg in messages:
                    ...
                    intro = msg.get("intro", "")
                    self._log(f"  message: subject={subject!r} intro={intro!r}")

                    # Fetch full message body for better OTP extraction
                    text = intro
                    if any(kw in subject.lower() for kw in ("twitter", "x.com", "confirm", "verify")):
                        try:
                            full = self._client.get(f"{BASE}/messages/{msg_id}", headers=headers)
                            text = full.json().get("text", intro) + " " + intro
                        except Exception:
                            pass
        ...
        raise MailTmError(f"OTP not received within {timeout}s at {mailbox.address}")
