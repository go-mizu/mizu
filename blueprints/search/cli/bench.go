...
func runBenchCompare(
	...
	fmt.Fprintf(os.Stderr, "  exact ordered top-%d (all): %d (%.2f%%)\n", limit, comp.Summary.ExactTopKAll, pctInt(comp.Summary.ExactTopKAll, comp.Summary.TotalQueries))
	fmt.Fprintf(os.Stderr, "  exact ordered top-%d (hit): %d (%.2f%%)\n", limit, comp.Summary.ExactTopKWithHitsEither, pctInt(comp.Summary.ExactTopKWithHitsEither, comp.Summary.QueriesWithHitsEither))
	fmt.Fprintf(os.Stderr, "  exact set top-%d (all):     %d (%.2f%%)\n", limit, comp.Summary.ExactSetTopKAll, pctInt(comp.Summary.ExactSetTopKAll, comp.Summary.TotalQueries))
	fmt.Fprintf(os.Stderr, "  exact set top-%d (hit):     %d (%.2f%%)\n", limit, comp.Summary.ExactSetTopKWithHits, pctInt(comp.Summary.ExactSetTopKWithHits, comp.Summary.QueriesWithHitsEither))
...
func printBenchReportSummary(file string, comp *bench.CompareResults, mismatchCount int, output string) {
	...
	fmt.Fprintf(os.Stderr, "  with hits (either):      %d\n", comp.Summary.QueriesWithHitsEither)
	fmt.Fprintf(os.Stderr, "  with hits (both):        %d\n", comp.Summary.QueriesWithHitsBoth)
	fmt.Fprintf(os.Stderr, "  exact ordered top-k (all): %d (%.2f%%)\n", comp.Summary.ExactTopKAll, pctInt(comp.Summary.ExactTopKAll, comp.Summary.TotalQueries))
	fmt.Fprintf(os.Stderr, "  exact ordered top-k (hit): %d (%.2f%%)\n", comp.Summary.ExactTopKWithHitsEither, pctInt(comp.Summary.ExactTopKWithHitsEither, comp.Summary.QueriesWithHitsEither))
	fmt.Fprintf(os.Stderr, "  exact set top-k (all):     %d (%.2f%%)\n", comp.Summary.ExactSetTopKAll, pctInt(comp.Summary.ExactSetTopKAll, comp.Summary.TotalQueries))
	fmt.Fprintf(os.Stderr, "  exact set top-k (hit):     %d (%.2f%%)\n", comp.Summary.ExactSetTopKWithHits, pctInt(comp.Summary.ExactSetTopKWithHits, comp.Summary.QueriesWithHitsEither))
	fmt.Fprintf(os.Stderr, "  avg overlap (all):       %.3f\n", comp.Summary.AvgOverlapAll)
	fmt.Fprintf(os.Stderr, "  avg overlap (hit queries): %.3f\n", comp.Summary.AvgOverlapWithHits)
	fmt.Fprintf(os.Stderr, "  overlap p50/p90/p99:     %d / %d / %d\n", comp.Summary.OverlapP50, comp.Summary.OverlapP90, comp.Summary.OverlapP99)
	fmt.Fprintf(os.Stderr, "  different hit count:     %d\n", comp.Summary.DifferentHitCount)
...
func buildBenchReportMarkdown(
	...
	b.WriteString("| Exact ordered top-k (all) | " + strconv.Itoa(comp.Summary.ExactTopKAll) + " (" + fmt.Sprintf("%.2f%%", pctInt(comp.Summary.ExactTopKAll, comp.Summary.TotalQueries)) + ") |\n")
	b.WriteString("| Exact ordered top-k (hit queries) | " + strconv.Itoa(comp.Summary.ExactTopKWithHitsEither) + " (" + fmt.Sprintf("%.2f%%", pctInt(comp.Summary.ExactTopKWithHitsEither, comp.Summary.QueriesWithHitsEither)) + ") |\n")
	b.WriteString("| Exact set top-k (all) | " + strconv.Itoa(comp.Summary.ExactSetTopKAll) + " (" + fmt.Sprintf("%.2f%%", pctInt(comp.Summary.ExactSetTopKAll, comp.Summary.TotalQueries)) + ") |\n")
	b.WriteString("| Exact set top-k (hit queries) | " + strconv.Itoa(comp.Summary.ExactSetTopKWithHits) + " (" + fmt.Sprintf("%.2f%%", pctInt(comp.Summary.ExactSetTopKWithHits, comp.Summary.QueriesWithHitsEither)) + ") |\n")
