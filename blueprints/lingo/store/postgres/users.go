...
func (s *UserStore) GetByID(ctx context.Context, id uuid.UUID) (*store.User, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id, email, username, display_name, avatar_url, bio, encrypted_password, xp_total, gems, hearts, hearts_updated_at, streak_days, streak_updated_at, streak_freeze_count, is_premium, premium_expires_at, daily_goal_minutes, created_at, last_active_at
		FROM users WHERE id = $1
	`, id).Scan(&user.ID, &user.Email, &user.Username, &user.DisplayName, &user.AvatarURL, &user.Bio, &user.EncryptedPassword, &user.XPTotal, &user.Gems, &user.Hearts, &user.HeartsUpdatedAt, &user.StreakDays, &user.StreakUpdatedAt, &user.StreakFreezeCount, &user.IsPremium, &user.PremiumExpiresAt, &user.DailyGoalMinutes, &user.CreatedAt, &user.LastActiveAt)
	if err != nil {
