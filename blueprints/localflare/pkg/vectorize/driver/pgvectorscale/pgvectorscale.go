// Package pgvectorscale provides a PostgreSQL/pgvectorscale driver for the vectorize package.
// Import this package to register the "pgvectorscale" driver.
// pgvectorscale uses DiskANN indexing for high-performance vector search at scale.
...
func init() {
	driver.Register("pgvectorscale", &Driver{})
}

// Driver implements vectorize.Driver for pgvectorscale.
type Driver struct{}

// Open creates a new pgvectorscale connection.
...
func (d *Driver) Open(dsn string) (vectorize.DB, error) {
	...
	// Initialize schema with pgvectorscale extensions
...
func initSchema(pool *pgxpool.Pool) error {
	...
	_, err = pool.Exec(ctx, `CREATE EXTENSION IF NOT EXISTS vectorscale CASCADE`)
	if err != nil {
		return fmt.Errorf("failed to enable vectorscale extension: %w", err)
	}

	// Create metadata table
	_, err = pool.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS vectorscale_indexes (
	...
	_, err = pool.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS vectorscale_vectors (
			id TEXT NOT NULL,
...
// DB implements vectorize.DB for pgvectorscale.
type DB struct {
	pool *pgxpool.Pool
}

// CreateIndex creates a new vector index with DiskANN.
func (db *DB) CreateIndex(ctx context.Context, index *vectorize.Index) error {
	// Check if index exists
	var exists bool
	err := db.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM vectorscale_indexes WHERE name = $1)`, index.Name).Scan(&exists)
	if err != nil {
		return err
	}
	if exists {
	...
	_, err = db.pool.Exec(ctx, `
		INSERT INTO vectorscale_indexes (name, dimensions, metric, description, vector_count, created_at)
		VALUES ($1, $2, $3, $4, 0, $5)
...
// createDiskANNIndex creates a DiskANN index for efficient search.
func (db *DB) createDiskANNIndex(ctx context.Context, indexName string, metric vectorize.DistanceMetric) error {
	...
	safeIndexName := strings.ReplaceAll(indexName, "-", "_")
	safeIndexName = strings.ReplaceAll(safeIndexName, ".", "_")

	// Create DiskANN index
	createIdxSQL := fmt.Sprintf(`
		CREATE INDEX IF NOT EXISTS vectorscale_%s_diskann_idx
		ON vectorscale_vectors USING diskann (embedding %s)
		WHERE index_name = '%s'
	`, safeIndexName, opClass, indexName)

	_, err := db.pool.Exec(ctx, createIdxSQL)
	return err
}
...
func (db *DB) GetIndex(ctx context.Context, name string) (*vectorize.Index, error) {
	...
	err := db.pool.QueryRow(ctx, `
		...
		FROM vectorscale_indexes WHERE name = $1
...
func (db *DB) ListIndexes(ctx context.Context) ([]*vectorize.Index, error) {
	rows, err := db.pool.Query(ctx, `
		SELECT name, dimensions, metric, description, vector_count, created_at
		FROM vectorscale_indexes ORDER BY name
	`)
	if err != nil {
		return nil, err
	}
...
func (db *DB) DeleteIndex(ctx context.Context, name string) error {
	// Drop DiskANN index first
	safeIndexName := strings.ReplaceAll(name, "-", "_")
	safeIndexName = strings.ReplaceAll(safeIndexName, ".", "_")
	db.pool.Exec(ctx, fmt.Sprintf(`DROP INDEX IF EXISTS vectorscale_%s_diskann_idx`, safeIndexName))

	// Delete vectors
	db.pool.Exec(ctx, `DELETE FROM vectorscale_vectors WHERE index_name = $1`, name)

	// Delete index metadata
	result, err := db.pool.Exec(ctx, `DELETE FROM vectorscale_indexes WHERE name = $1`, name)
	if err != nil {
		return err
	}
...
func (db *DB) Insert(ctx context.Context, indexName string, vectors []*vectorize.Vector) error {
	...
	for _, v := range vectors {
		...
		_, err := tx.Exec(ctx, `
			INSERT INTO vectorscale_vectors (id, index_name, namespace, embedding, metadata)
			VALUES ($1, $2, $3, $4, $5)
	...
	// Update vector count
	_, err = tx.Exec(ctx, `
		UPDATE vectorscale_indexes SET vector_count = (
			SELECT COUNT(*) FROM vectorscale_vectors WHERE index_name = $1
		) WHERE name = $1
	`, indexName)
	if err != nil {
		return err
	...
	// Create DiskANN index after first batch insert (deferred to allow bulk loading)
	var count int64
	db.pool.QueryRow(ctx, `SELECT vector_count FROM vectorscale_indexes WHERE name = $1`, indexName).Scan(&count)
	if count > 0 && count <= int64(len(vectors)) {
		// First insert, create index
		db.createDiskANNIndex(ctx, indexName, idx.Metric)
	}
...
// Search finds similar vectors using DiskANN index.
func (db *DB) Search(ctx context.Context, indexName string, vector []float32, opts *vectorize.SearchOptions) ([]*vectorize.Match, error) {
	...
	// pgvectorscale uses same operators as pgvector
...
func (db *DB) Delete(ctx context.Context, indexName string, ids []string) error {
	...
	_, err = tx.Exec(ctx, `
		DELETE FROM vectorscale_vectors WHERE index_name = $1 AND id = ANY($2)
	`, indexName, ids)
	if err != nil {
		return err
	}
