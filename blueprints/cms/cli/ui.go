...
// Color palette (WordPress/CMS theme)
var (
	primaryColor   = lipgloss.Color("#21759B") // WordPress blue
	secondaryColor = lipgloss.Color("#6B7280") // Gray
	accentColor    = lipgloss.Color("#0073AA") // Link blue
...
func (u *UI) UserRow(username, email, role string) {
	fmt.Printf("  %s %-20s %-30s %s\n",
		iconUser,
		userStyle.Render(username),
		valueStyle.Render(email),
		subtitleStyle.Render("["+role+"]"))
}

// PostRow prints a formatted post row
func (u *UI) PostRow(title, status, author string) {
	if len(title) > 40 {
		title = title[:40] + "..."
	}
	fmt.Printf("  %s %-45s %-10s by %s\n",
		iconPost,
		postStyle.Render(title),
		subtitleStyle.Render("["+status+"]"),
		userStyle.Render(author))
}
