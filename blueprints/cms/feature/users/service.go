...
func (s *Service) GetBySlug(ctx context.Context, userSlug string) (*User, error) {
	user, err := s.store.GetBySlug(ctx, userSlug)
	if err != nil {
		return nil, err
	}
	if user == nil {
...
func (s *Service) List(ctx context.Context, in *ListIn) ([]*User, int, error) {
	if in.Limit <= 0 {
		in.Limit = 20
	}
