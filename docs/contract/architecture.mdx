...
│  │  func (s *TodoService) Create(...) (*Todo, error)              │  │
...
| tRPC | Path-based + JSON body | tRPC envelope |
...
The REST handler (mounted by `contract.MountREST()`) receives the HTTP request:
...
The contract package is organized into:

```
contract/
├── contract.go          # Core: Service, Method, Register
├── types.go             # TypeRegistry, JSON schema generation
├── errors.go            # Error types and codes
├── client.go            # Client-side utilities
│
├── transport_rest.go    # REST transport (MountREST)
│
└── transport/           # Additional transports
    ├── jsonrpc/         # JSON-RPC 2.0
    │   ├── jsonrpc.go   # Request/Response types
    ...
    │
    ├── trpc/            # tRPC-like
    │   ├── trpc.go      # Envelope types
    │   └── handler.go   # HTTP handler
    │
    └── rest/            # REST utilities
        └── spec.go      # OpenAPI generation
```
