...
Use `Errorf` when you need to include variables:
...
Use these when the client made a mistake:
...
func (s *todoService) Get(ctx context.Context, in *GetInput) (*Todo, error) {
    ...
    if err != nil {
        ...
        // Other errors -> preserve original for logging
...
// API response shows: "database error"
```
...
func (s *userService) Get(ctx context.Context, in *GetInput) (*User, error) {
    if in.ID == "" {
...
func (s *userService) Delete(ctx context.Context, in *DeleteInput) error {
...
func (s *userService) insertUser(ctx context.Context, in *CreateInput) (*User, error) { ... }
...
func TestCreate_Validation(t *testing.T) {
    ...
    tests := []struct {
        ...
        {"empty email", &CreateInput{Email: ""}, contract.ErrCodeInvalidArgument},
        {"bad email", &CreateInput{Email: "not-email"}, contract.ErrCodeInvalidArgument},
        {"short password", &CreateInput{Email: "a@b.com", Password: "short"}, contract.ErrCodeInvalidArgument},
    }
...
Your service should return errors, not log them. Let middleware or the transport layer handle logging:

```go
// In service - just return the error
