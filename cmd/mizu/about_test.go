...
// A file that cannot be opened is not evidence of anything, and a command that
// reports what a project is made of has no business ending over one.
func TestGeneratedInWalksPastWhatItCannotRead(t *testing.T) {
	got, err := generatedIn(refusing{fstest.MapFS{
...
// refusing is a file system whose files cannot be opened. Reading a directory
// still works, so the walk gets as far as the files and no further.
type refusing struct{ fs.FS }

func (r refusing) Open(name string) (fs.File, error) {
	if f, err := r.FS.Open(name); err == nil {
