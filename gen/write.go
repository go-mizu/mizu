...
func Header(generator, version, source string) string {
	...
	for _, line := range headerLines(generator, version, source) {
		b.WriteString("// ")
		b.WriteString(line)
		b.WriteString("\n")
	}
...
// HTMLHeader returns the same header for a file whose only comment is an HTML
// one, which is Markdown as well as HTML.
//
//	<!-- Code generated by mizu gen agents v1; DO NOT EDIT.
//	     Source: go.mod -->
//
// The marker has to be the whole of its line, so the comment is closed at the
// end of the last one rather than at the end of the first.
func HTMLHeader(generator, version, source string) string {
	lines := headerLines(generator, version, source)
	var b strings.Builder
...
// headerLines is the header without the comment marker, which is the only part
// that depends on the language the file is written in.
func headerLines(generator, version, source string) []string {
	var b strings.Builder
...
func Generated(data []byte) bool {
	_, ok := generatedHeader(data)
	return ok
}

// GeneratedBy returns the tool named in the header, or the empty string when
// there is no header or it says nothing about who wrote the file.
//
// The rule in [Generated] fixes the two ends of the line, so what is between
// them is whatever the tool decided to say about itself. This takes it as it
// is, without the "by" that most of them write, so a project that runs sqlc or
// protoc gets those names alongside mizu's own.
func GeneratedBy(data []byte) string {
	by, ok := generatedHeader(data)
	if !ok {
		return ""
	}
	by = strings.TrimSuffix(strings.TrimSpace(by), ";")
	by = strings.TrimPrefix(strings.TrimSpace(by), "by ")
	return strings.TrimSuffix(strings.TrimSpace(by), ".")
}

// generatedHeader returns what the marker line says between the two ends the
// rule fixes, and whether the file had such a line at all.
func generatedHeader(data []byte) (string, bool) {
	for len(data) > 0 {
		line := data
		...
		switch middle, ok := generatedLine(text); {
		case ok:
			return middle, true
...
// generatedLine reports whether text is the line that marks a file as
// generated, and returns what it says between the marker and the suffix.
//
// An HTML comment that fits on one line closes after the suffix. Taking the
// closer off first is what lets the rule hold for it too.
func generatedLine(text string) (string, bool) {
	text = strings.TrimSuffix(text, " -->")
	if !strings.HasSuffix(text, generatedSuffix) {
		return "", false
	}
	for _, opener := range commentOpeners {
		prefix := opener + " " + generatedMarker
		if !strings.HasPrefix(text, prefix) {
			continue
		}
		// The marker and the suffix can overlap on the space between them, in
		// a line that names nobody at all. It is still a generated file, and
		// there is still nothing in the middle to return.
		if end := len(text) - len(generatedSuffix); end > len(prefix) {
