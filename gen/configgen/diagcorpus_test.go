...
//
// Each directory under testdata/_diag holds a config.go that is wrong in one
// way, and a want.txt, which is what somebody running mizu gen over it sees.
// A configuration struct is written once and read by everybody, so the message
// that refuses one is read by somebody who did not write it.
//
...
func TestEveryMessageHasAnEntry(t *testing.T) {
	...
	// of the field in front of what checkDefault said about its default. Both
	// of those messages are in the corpus under their own text.
