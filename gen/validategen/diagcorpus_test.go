...
//
// Each directory under testdata/_diag holds a validate.go whose tags are wrong
// in one way, and a want.txt, which is what somebody running mizu gen over it
// sees. A generator that refuses code is talking to somebody who has to change
// that code, and the message is the whole of what they have to go on.
//
...
func TestEveryMessageHasAnEntry(t *testing.T) {
	// The one that is skipped is a wrapper rather than a message. errf writes
	// the position in front of what it was handed, and every message that goes
	// through it is in the corpus under its own text.
	diagtest.Cover(t, corpus, ".", "%s: %s")
}
