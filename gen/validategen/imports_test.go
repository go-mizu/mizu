...
func TestImportName(t *testing.T) {
	...
	cases := []struct {
		...
		{"github.com/go-mizu/mizu/validate", "validate"},
	}
	for _, c := range cases {
...
func TestImportLines(t *testing.T) {
	i := newImports("example.com/app")
	for _, pkg := range []string{"time", "net/netip", "example.com/other/time", "github.com/go-mizu/mizu/validate"} {
		i.name(pkg)
	}
...
// A struct whose only rules are required and a format check imports the
// validate package and nothing else, which reads better on one line.
func TestWriteOneImport(t *testing.T) {
	var b strings.Builder
	writeImports(&b, []importLine{{Path: "github.com/go-mizu/mizu/validate"}})

	want := "import \"github.com/go-mizu/mizu/validate\"\n\n"
	if b.String() != want {
...
func TestPathBase(t *testing.T) {
	cases := map[string]string{
		...
		"example.com/values":    "values", // a v is not a version without digits after it
		"example.com/v":         "v",
	...
	for pkg, want := range cases {
		if got := pathBase(pkg); got != want {
			t.Errorf("pathBase(%q) = %q, want %q", pkg, got, want)
		}
	}
}
...
func TestIsStd(t *testing.T) {
	cases := map[string]bool{
		...
		"net/netip":                        true,
		"mime/multipart":                   true,
		"github.com/go-mizu/mizu/validate": false,
