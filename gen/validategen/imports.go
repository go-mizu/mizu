...
// appears in a message and nowhere else must not drag in an import that nothing
// then uses, because an unused import does not compile.
type imports struct {
...
//
// A package's own name is usually the last element of its path, and this is the
// form for the handful of paths written into this generator by hand, where that
// is known to hold. Anything read out of a loaded program goes through
// [imports.pkg] instead, which does not have to guess.
func (i *imports) name(pkg string) string { return i.named(pkg, pathBase(pkg)) }
...
func (i *imports) lines() []importLine {
	...
	for pkg, name := range i.names {
		...
		if name != pathBase(pkg) {
			// Only say the name when the import would not give it anyway, which
			// is when two packages wanted one name or when a package is not
			// called what its path ends in.
			line.Alias = name
		}
		if isStd(pkg) {
			std = append(std, line)
...
// docString writes a type for somebody to read, without asking for a single
// import, because this one ends up in an error message.
...
// pathBase is the name an import of this path gives, which is the last element,
